...
static void _glfwSendJoyReadRequest(void)
{
    ...
    IExec->SendIO((struct IORequest *)_glfwJoy.GameIO);
}
...
void _glfwInitJoysticks(void)
{
    ...
    if( !(_glfwJoy.GameMP = CreateMsgPort()) )
    {
        return;
    }
    ...
    if( !( _glfwJoy.GameIO = (struct IOStdReq *)
           CreateIORequest(_glfwJoy.GameMP, sizeof(struct IOStdReq)) ) )
    {
    ...
    if( IExec->OpenDevice( "gameport.device", 1,
...
void _glfwTerminateJoysticks(void)
{
    ...
    if (_glfwJoy.GameIO)
    {
        if (!IExec->CheckIO((struct IORequest *)_glfwJoy.GameIO))
        {
            IExec->AbortIO((struct IORequest *)_glfwJoy.GameIO);
            IExec->WaitIO((struct IORequest *)_glfwJoy.GameIO);
        }
    }
    ...
    if (_glfwJoy.Present)
    {
        ...
        _glfwJoy.GameIO->io_Data = (APTR)&controller_type;
        _glfwJoy.GameIO->io_Length = 1;
        IExec->DoIO((struct IORequest *)_glfwJoy.GameIO);
        _glfwJoy.Present = 0;
    }
    ...
    {
        IExec->CloseDevice((struct IORequest *)_glfwJoy.GameIO);
        _glfwJoy.GameDeviceOpen = 0;
    }
    ...
    if (_glfwJoy.GameIO)
    {
        IExec->DeleteIORequest((struct IORequest *)_glfwJoy.GameIO);
        _glfwJoy.GameIO = NULL;
    }
    ...
    if (_glfwJoy.GameMP)
    {
        IExec->DeletePort(_glfwJoy.GameMP);
        _glfwJoy.GameMP = NULL;
    }
}
...
static void _glfwPollJoystickEvents(void)
{
    ...
    while (IExec->GetMsg(_glfwJoy.GameMP) != NULL)
    {
